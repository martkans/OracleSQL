...
select k.DAJ_WLASCICIELA().DAJ_ELITE().pseudo        "Pseudo",
...
DECLARE
    pseudonim        KOCURY_R.PSEUDO%TYPE;
    przydzial        KOCURY_R.PRZYDZIAL_MYSZY%TYPE;
    im               KOCURY_R.IMIE%TYPE;
    mies             NUMBER;
...
declare
    cursor naj_przydz is
        ...
        from (
                 select PSEUDO,
                        R.DAJ_CALKOWITY_PRZYDZIAL_MYSZY()                                   cal_przydz,
                        DENSE_RANK() over (order by R.DAJ_CALKOWITY_PRZYDZIAL_MYSZY() desc) poz
                 from KOCURY_R R
             )
        where poz < 6;
begin
